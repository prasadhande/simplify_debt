module DebtAction
  ...
  def store_payment_due(data)
    hash_values = data[:simplified_view]
    hash_values.each do |payment_data|
      Payment.create(group_member_id: payment_data[:member_id], owe_to_group_member_id: payment_data[:owe_to_id], amount: payment_data[:amount])
    end
  end
end